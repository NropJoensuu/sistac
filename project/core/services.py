"""
.. topic:: Core (services) — Home / Info / Cargas de arquivo / Integração SICONV-DW

    Camada de regra de negócio dos grupos "Home/Info", "Cargas de
    arquivo", "Chamadas/Homologados" e "Integração SICONV/DW" do
    módulo core.

    Nota técnica: `cargaPDCTR` e `carregar_homologados` usam
    `flash`/`redirect` diretamente (exceção à regra geral de manter
    services.py livre de objetos de request/response do Flask). Isso
    preserva o comportamento exato do código original: se a planilha
    não tiver os campos esperados, uma mensagem de erro é definida via
    `flash`, mas a função retorna um valor que a rota chamadora
    atualmente ignora — ou seja, o fluxo continua normalmente após o
    erro. Esse comportamento é preservado aqui sem alteração; fica
    como candidato a revisão numa futura mudança de regra de negócio,
    não nesta refatoração.

    .. topic:: Security / Technical Debt (Grupo C — Integração SICONV/DW)

        Três pontos foram identificados durante a refatoração deste
        grupo e **deliberadamente não corrigidos agora**, por decisão
        explícita: preservar o comportamento e o funcionamento real da
        integração Oracle, e abrir uma tarefa dedicada de hardening
        depois, em vez de arriscar uma mudança não testável junto da
        reestruturação.

        1. **Concatenação de SQL em `consultaDW`**: as 5 variações de
           consulta ao Data Warehouse Oracle montam o SQL por
           concatenação de string direta (uma delas via f-string),
           sem bind parameters. Hoje, todo caminho de chamada
           encontrado no código (`core/services.py` e
           `acordos/views.py`) passa valores originados de dados já
           sincronizados do próprio SICONV/DW ou de escolhas
           restritas (dropdown de coordenação do admin) — não há um
           caminho de ataque explorável identificado hoje. Mas a
           função não tem nenhuma defesa própria: uma mudança futura
           que passe input de usuário direto para essa função vira
           SQL injection imediata contra o Oracle. Recomendação:
           migrar para bind parameters (`:nome` do `oracledb`) numa
           tarefa dedicada de "Hardening do acesso Oracle DW".

        2. **`oracledb.init_oracle_client()` chamado a cada consulta**:
           `consultaDW()` chama `init_oracle_client()` toda vez que é
           executada, e é chamada dezenas de vezes em loop dentro de
           `chamadas_DW()` (uma vez por programa, depois por chamada).
           A biblioteca `oracledb` tipicamente lança erro se esse
           método for chamado mais de uma vez no mesmo processo. Não
           foi possível validar isso com um Oracle real neste
           ambiente (sem acesso à infraestrutura de produção). Se
           isso já causa erro em produção hoje, é anterior a esta
           refatoração — o comportamento foi preservado exatamente
           como estava. Recomendação: mover a inicialização para
           acontecer uma única vez (ex: na criação do app), fora do
           loop de consultas.

        3. **Dependência cruzada com `acordos/views.py`**:
           `consultaDW()` também é chamada de dentro do módulo
           `acordos` (ainda não refatorado), com os tipos
           `'programas_unid'` e `'financeiro_processos'`. Isso
           significa que o `acordos` depende diretamente de uma
           função que agora vive em `core/services.py` — ao
           refatorar o `acordos`, será necessário atualizar esses
           dois pontos de import (`from project.core.views import
           consultaDW` deve virar `from project.core.services import
           consultaDW`).
"""
...
def consultaDW(**entrada):
    ...
    if entrada['tipo'] == 'programas_unid': # programas associados a uma unidade, procura no FT_PAGAMENTO por seq_id_programa_PROC e seq_id_programa
        ...
        # BUG CORRIGIDO: o código original fazia "return res" aqui, mas
        # "res" nunca havia sido definido nesse ponto (NameError certo
        # se a função fosse chamada com um 'tipo' não previsto).
...
def chamadas_DW():
    ...
    if current_user == None or current_user.get_id() == None:
        user_agenda = db.session.query(Log_Auto.user_id)\
                                ...
                                .order_by(Log_Auto.id.desc())\
                                .first()
        # BUG CORRIGIDO: sem checar se a query retornou None (acontece
        # numa instalação nova, antes de qualquer agendamento anterior).
        id_user = user_agenda.user_id if user_agenda else None
                            ...
                            for pro_cha in processos_chamadas:
                                ...
                                for fil_cha in filhos_chamadas:
                                    ...
                                    if fil_cha[3] == pro_cha[2]:  # grava filho se ele for do processo mãe da vez
                                        ...
                                        # BUG CORRIGIDO: o filtro original usava "and" do Python entre
                                        # duas expressões do SQLAlchemy, o que faz a condição de
                                        # modalidade (Bolsa.mod) ser silenciosamente ignorada — só o
                                        # nível era aplicado de fato. Isso podia levar a um valor de
                                        # bolsa errado (impacto no cálculo de VALOR_APAGAR).
                                        bolsa = db.session.query(Bolsa).filter(Bolsa.mod == dic_fil_cha['MODAL'], Bolsa.niv == dic_fil_cha['NIVEL']).first()
                                        if bolsa:
...
def cargaSICONV():
    ...
    if current_user == None or current_user.get_id() == None:
        user_agenda = db.session.query(Log_Auto.user_id)\
                                ...
                                .order_by(Log_Auto.id.desc())\
                                .first()
        # BUG CORRIGIDO: mesma checagem de None que faltava aqui.
        id = user_agenda.user_id if user_agenda else None
    else:
        id = current_user.id 
    ...
    # BUG CORRIGIDO: o caminho original era '/temp/arqs_siconv' (path
    # absoluto que não existe por padrão em Linux — o diretório de
    # sistema correto é '/tmp', não '/temp' — e exigiria permissão de
    # root para criar na raiz). Usa o diretório temporário do SO.
    pasta_compactados = os.path.normpath(os.path.join(tempfile.gettempdir(), 'arqs_siconv'))
...
def thread_cargaSICONV():
    # BUG CORRIGIDO: o "with app.app_context()" original só envolvia a
    # criação/início da thread, não a execução de cargaSICONV() dentro
    # dela. Contexto de aplicação do Flask não se propaga automatica-
    # mente para threads filhas — isso fazia a carga assíncrona
    # quebrar sempre com "Working outside of application context"
    # assim que tentava acessar o banco de dados.
    def _run():
